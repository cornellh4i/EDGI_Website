...
def get_cwa_df(df):
    ...
    d = df.groupby(pd.to_datetime(
        df["YEAR"], format="%Y").dt.to_period("Y")).sum()
...
def get_top_violators(df_active, flag, noncomp_field, action_field, num_fac=10):
    ...
    df_active = df_active[
        ["FAC_NAME", "noncomp_count", action_field,
...
def build_all_per_1000(total_df):
    ...
    cd_per_1000 = total_df[total_df['Region']
                           == 'Congressional District'].copy()
    cd_per_1000['CAA_Insp_Pct'] = (cd_per_1000['CAA.Insp.per.1000'] /
                                   cd_per_1000['CAA.Viol.per.1000']).rank(pct=True)
    cd_per_1000['CAA_Viol_Pct'] = cd_per_1000['CAA.Viol.per.1000'].rank(
        pct=True)
    ...
    cd_per_1000['CWA_Viol_Pct'] = cd_per_1000['CWA.Viol.per.1000'].rank(
        pct=True)
    ...
    cd_per_1000['RCRA_Viol_Pct'] = cd_per_1000['RCRA.Viol.per.1000'].rank(
        pct=True)
    ...
    county_per_1000['CAA_Viol_Pct'] = county_per_1000['CAA.Viol.per.1000'].rank(
        pct=True)
    ...
    county_per_1000['CWA_Viol_Pct'] = county_per_1000['CWA.Viol.per.1000'].rank(
        pct=True)
    ...
    county_per_1000['RCRA_Viol_Pct'] = county_per_1000['RCRA.Viol.per.1000'].rank(
        pct=True)
